class DropBoxController{
  ...
  var firebaseConfig = {
    apiKey: "AIzaSyDWWCyKA726ZaYOloPN-zgOzS_YGnoPtdA",
    authDomain: "dropbox-clone-c38ee.firebaseapp.com",
    databaseURL: "https://dropbox-clone-c38ee.firebaseio.com",
    projectId: "dropbox-clone-c38ee",
    storageBucket: "dropbox-clone-c38ee.appspot.com",
    messagingSenderId: "26812152830",
    appId: "1:26812152830:web:f075cba30bab874ac89eaf",
    measurementId: "G-XBJ4TWR5X3"
  };
  // Initialize Firebase
  ...
  firebase.analytics();
    ...
    renderNav(){
        ...
        for(let i=0;i<this.currentFolder.length-1;i++){
            ...
            span.innerHTML=`<span class="ue-effect-container uee-BreadCrumbSegment-link-0">
                <a href="https://www.dropbox.com/work" data-path="${path.join('/')}"class="breadcrumb-segment">${folderName}</a>
            </span>
        ...
        let span = document.createElement('span');
        let folderName = this.currentFolder[this.currentFolder.length-1];
        path.push(folderName);
    ...
    initEvents(){
        ...
        this.inputFilesEl.addEventListener('change',event=>{
            ...
            this.uploadTask(event.target.files).then(responses=>{
                ...
                responses.forEach(resp => { resp.ref.getDownloadURL().then(data=>{ this.getFirebaseRef().push().set({ name: resp.name, type: resp.contentType, path: data, size: resp.size }); });});
                this.uploadComplete();
            }).catch(err=>{
                this.uploadComplete();
                console.error(err);
    ...
    initEventsLi(li){
        ...
        li.addEventListener('click',e=>{
            ...
            if(!e.ctrlKey){
                this.listFilesEl.querySelectorAll("li.selected").forEach(li=>{
